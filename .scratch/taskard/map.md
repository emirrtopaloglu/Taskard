...
- **Lean Damıtma, Point-to-Range & Akıllı Model Tiering (2026-08-29):** Token tasarrufu, context temizliği ve hız artışı için 5 maddelik mimari optimizasyon:
  1. **Core SKILL.md Damıtıldı:** 4.5k token'dan ~2.3k token'a indirildi; statik kurulum, hafıza ve cross-harness detayları `references/` klasörüne taşındı (progressive disclosure).
  2. **Yerleşik TDD & Kanıt Sözleşmesi:** `test-driven-development` ve `verification-before-completion` dış skill bağımlılıkları kaldırıldı; Red-Green-Refactor ve somut komut kanıtı disiplini doğrudan `implementer.md` içine yerleşik olarak gömüldü.
  3. **Express Modda Akıllı Model Tiering:** `reviewer` ve `debugger` rolleri Express modda orta modele (`sonnet`) çekildi; `opus` yalnızca Full modda derin mimari/güvenlik için ayrıldı.
  4. **Self-Priming Brief'lerde Point-to-Range Standardı:** Brief'e kod kopyalama yasaklandı; yalnızca hedef dosya ve satır aralığı pointer'ı (`file#L10-L40`) verilir, subagent yalnızca o satırları okur.
  5. **Agresif Nano Modu:** Tek dosya/typo/stil/küçük fix'lerde sıfır dosya ve anında ana döngü doğrulamasıyla Nano birincil hızlı tercih haline getirildi.
  Emir onayı.
